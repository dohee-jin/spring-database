...
@SpringBootTest
@Transactional // 롤백이 포함되어 있음, 연관관계 사용시 필수
...
class EmployeeRepositoryTest {
    ...
    void insertBulk() {
        ...
        List<Department> departments = departmentRepository.saveAllAndFlush(List.of(d1, d2));
        ...
        employeeRepository.saveAllAndFlush(List.of(e1, e2, e3, e4));

        /*
            JPA 는 영속성 컨텍스트라는 대형 데이터 창고를 운영한다.
    ...
    @Test
    @DisplayName("특정 사원의 정보를 조회함변 부서정보도 같이 조회된다.")
    void findTest() {
        // given
        Long empId = 2L;
        // when
        ...
        // 명시적으로 부서 정보를 참조 - LAZY 로딩은 이 시점에 부서정보를 가져옴
...
//        employeeRepository.save(employee);
    ...
    void nPlusOneTest() {
        ...
        departments.forEach(dept -> {
            System.out.println("dept + dept.getEmployees()= " + dept + dept.getEmployees());
        });
    }
